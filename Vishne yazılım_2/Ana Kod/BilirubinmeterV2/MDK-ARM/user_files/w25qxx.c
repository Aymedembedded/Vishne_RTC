...
extern uint16_t VIDEO_DATA[ 256];// silinecek!!!!!!
uint16_t PatientCounter[1];
char PatientAscii[10];
uint16_t ReadSectorBuffer[256];
uint32_t	id;
...
#define	W25qxx_Delay(delay)		DWT_Delay_us(delay*1000)
//#include "cmsis_os.h"
#else
...
//###################################################################################################################
void SaveSpectrumToSector(void)
{
	...
	if( Sector_Addr >= StartMem && Sector_Addr <=EndMem)
	{
		...
		W25qxx_WriteSector((uint8_t *)VIDEO_DATA,Sector_Addr,0,512);
	}
	...
	//my_itoa(PatientAscii,(PatientCounter[0]-1619));
}


//###################################################################################################################
void EraseSectors (uint32_t StartSector_Addr , uint32_t ToSector_Addr )
{
	for(int erase=StartSector_Addr ; erase<ToSector_Addr ; erase++)
	{
		W25qxx_EraseSector(erase);	
		W25qxx_EraseSector(DateMemory + (ToSector_Addr - StartSector_Addr));
		W25qxx_EraseSector(TimeMemory + (ToSector_Addr - StartSector_Addr));
	}	
	
	PatientCounter[0] = StartMem;
	W25qxx_WriteSector((uint8_t *)PatientCounter,PtnMEM,0,2);
	W25qxx_ReadSector((uint8_t *)PatientCounter,PtnMEM,0,2);	
}
...
void TransferData( void)
{
	...
	for(int snd=StartMem ; snd<SctrAdrr ; snd++)
	{
		if(snd < EndMem)
		{
			W25qxx_ReadSector((uint8_t *)ReadSectorBuffer,snd,0,512);	
		
			HAL_UART_Transmit_IT(&huart3,(uint8_t  *)ReadSectorBuffer,512);
	...
	/*char seperatorBuffer[9];
	sprintf(seperatorBuffer,"");
		
	HAL_UART_Transmit_IT(&huart3,(uint8_t  *)seperatorBuffer,sizeof(seperatorBuffer));
		
	HAL_Delay(1000);*/

	
	uint8_t ReadSectorBufferforInfo[9];
	for(int snd=0 ; snd < 9 * (measurementNumber - 1); snd = snd + 9)
	{
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo, eepromMemoryinByte - (snd));
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo+1, eepromMemoryinByte - (snd+1));
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo+2, eepromMemoryinByte - (snd+2));
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo+3, eepromMemoryinByte - (snd+3));
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo+4, eepromMemoryinByte - (snd+4));
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo+5, eepromMemoryinByte - (snd+5));
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo+6, eepromMemoryinByte - (snd+6));
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo+7, eepromMemoryinByte - (snd+7));
			W25qxx_ReadByte((uint8_t *)ReadSectorBufferforInfo+8, eepromMemoryinByte - (snd+8));
		
			HAL_UART_Transmit_IT(&huart3,(uint8_t  *)ReadSectorBufferforInfo,sizeof(ReadSectorBufferforInfo));
...
//###################################################################################################################	
void SaveBlcRef( void)
{
	W25qxx_EraseSector(1621);
	W25qxx_WriteSector((uint8_t *)VIDEO_DATA,1621,0,512);
}
//###################################################################################################################	
void SaveWhtRef( void)
{
	W25qxx_EraseSector(1622);
	W25qxx_WriteSector((uint8_t *)VIDEO_DATA,1622,0,512);
}
...
void writeBilirubintoMemory(float bilirubinValue)
{
	uint8_t biliru[3];

	float bilirubinTemp;
	biliru[0] = (uint8_t)bilirubinValue;
	bilirubinTemp = bilirubinValue - (uint16_t)bilirubinValue;
	bilirubinTemp = bilirubinTemp * 100;
	biliru[1] = (uint8_t)(bilirubinTemp);
	bilirubinTemp = bilirubinTemp - biliru[1];
	bilirubinTemp = bilirubinTemp * 10;
	biliru[2] = (uint8_t)(bilirubinTemp);
	
	uint16_t measurementNo;
			
	measurementNo = measurementNumber - 1;
	measurementNo = 9 * measurementNo;

	for(int i=6; i < 9; i++)
	{
		W25qxx_WriteByte(biliru[i-6], eepromMemoryinByte - (measurementNo+i));
	}
